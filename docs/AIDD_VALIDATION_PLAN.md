# Validation plan: a TC-based druggability filter
...
**Hypothesis.** Among surface-exposed pockets of comparable geometry, those with **low TC**
(rigid, well-coupled neighbourhood) are more likely to coincide with a true ligand-binding
site than those with **high TC** (soft, flexible neighbourhood).

Two specific claims have to be separated, because they can fail independently:

1. **TC adds signal.** Within a stratum of matched SASA and volume, pockets containing the
   true ligand have lower TC than pockets that do not.
2. **TC adds signal beyond SASA.** The T C term improves ranking over a geometry-only score
...
false" means TC explains part of pocket quality but is redundant with geometry, and the README
should say so.
...
Geometry comes from an existing detector, not from TC. Two options, in order of preference:
...
tercile, compare the z-scored mean TC of hit versus non-hit pockets (Mann-Whitney, effect
size). The hypothesis predicts hits have systematically lower TC.

**Step 2 — incremental value (claim 2).** Fit a logistic model per pocket:

```
hit ~ SASA + volume                     (geometry only)
hit ~ SASA + volume + TC_z              (geometry + TC)
```
...
the out-of-fold predictions. The deliverable is one number: the AUC gained by adding TC.
...
| --- | --- | --- |
| hits show lower TC within SASA terciles, and adding TC raises grouped AUC by ≥ 0.03 with a non-significant shuffled control | filter validated | move the module in the README from "proposed" to "validated", add the triage figure |
| TC difference present but AUC gain < 0.03 | mechanism real, product value not demonstrated | keep the filter as a secondary annotation, state the redundancy with geometry |
| no TC difference within SASA terciles | hypothesis fails | remove the druggability-filter section, keep the "softness indicator" framing of the original README |
...
structure, TC in under a second. The whole COACH420 benchmark is a few hours of compute and no
...
* **Ligand size.** Larger ligands contact more residues and mechanically raise the number of
  rigid contacts. Check that the TC effect survives a ligand-heavy-atom-count covariate.
...
* **Resolution.** Low-resolution structures have noisier C-alpha positions; check that TC
