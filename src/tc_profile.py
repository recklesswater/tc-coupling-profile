...
def parse_ca(path: str, pdb_id: str = "structure"):
    """Return (ca_coords, residue_names, sasa) for the first chain."""
    ...
    for res in chain:
        if res.id[0] != " " or "CA" not in res:
            continue
        ca.append(res["CA"].coord.copy())
        names.append(res.resname)
        sasa.append(getattr(res, "sasa", np.nan))
    return np.asarray(ca, float), np.asarray(names), np.asarray(sasa, float)
